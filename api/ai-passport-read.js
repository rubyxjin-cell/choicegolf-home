...
[{"image_index": 1, "eng_name": "성/이름 (예: HONG GILDONG)", "kor_name": "한글 성명 (여권의 한글 성명란)", "passport_no": "여권번호", "birth_date": "YYYY-MM-DD", "expiry_date": "YYYY-MM-DD", "nationality": "국적코드 (예: KOR)", "sex": "M 또는 F", "uncertain": ["흐릿하거나 확신이 없는 필드명 목록 (eng_name/passport_no/birth_date/expiry_date 중)"]}]
...
- 여권이 아닌 사진이면 모든 값을 "" 로.`
    ...
    const results = urls.map((url, i) => {
      if (!images[i]) return { url, eng_name: '', kor_name: '', passport_no: '', birth_date: '', expiry_date: '', nationality: '', sex: '', uncertain: [], error: '이미지 로드 실패' };
      // 유효 이미지 중 몇 번째인지 계산
      ...
      return {
        ...
        kor_name: String(hit.kor_name || '').trim(),
...
{"eng_name": "성 이름 (대문자)", "kor_name": "한글 성명", "passport_no": "여권번호", "birth_date": "YYYY-MM-DD", "expiry_date": "YYYY-MM-DD", "nationality": "KOR", "sex": "M 또는 F", "uncertain": ["확신 없는 필드명"]}
